...
extern "C" {
    bool cuda_asteroidPos(float* outputData, size_t width, size_t height, size_t theta,
        float factor);
}
...
int main()
{
    ...
    std::cout << "CUDA Cores: " << cudaInfo[0];
    ...
    float asteroidAngle = pi - (std::rand() % 20) * pi / 180;
    ...
    while (window.isOpen())
    {
        ...
        {
            float asteroidAngle = (pi / 180) * (asteroidArr[i].getRotation()); // Convert asteroid's angle to radians
            //=>Movement
...
//
//
//void RunKernels(std::vector<Asteriod> asteroidArr) {
//
...
//        float* p;
//        float pos[2] = {
//            asteroidArr[0].getPosition().x,
//            asteroidArr[0].getPosition().y
//        };
//
//        p = pos;
//
//        cuda_asteroidPos(p, 0, 0, asteroidArr[0].getRotation(), 1);
//        getLastCudaError("cuda_asteroidPos failed");
//
